...
Section~II lays out the foundations: PMU measurement physics, CNN-LSTM architecture principles, and the baselines against which we measure. Section~III walks through System-1 in detail---model design, our decision to operate on raw features rather than hand-crafted ones, the multi-objective training protocol, and the inference pipeline including projected edge deployment performance. Section~IV covers our experimental methodology: test systems, fault injection protocol, baseline selection, and evaluation metrics. Section~V presents results: per-fault-type accuracy, latency characterization (measured GPU and projected Pi~5), baseline comparison, scalability analysis, adversarial robustness, and ablation studies. Section~VI steps back to ask what the results mean---architectural insights, limitations we cannot dodge, hardware practicalities, and integration pathways with existing protective relay infrastructure. Section~VII closes with contributions, implications, and the open questions we are still wrestling with. Appendices provide PyTorch pseudocode, hyperparameter tuning details, and a MATLAB implementation reference.
...
\textbf{Spatial Stream.} The multi-bus input is flattened into a single channel dimension of $(N_{\text{buses}} \times 3, 256)$ and passed through two one-dimensional convolutional layers. The first layer---64 filters, kernel size~3, same padding, ReLU activation---produces output of shape (64, 256). Max-pooling with pool size~2 reduces the temporal dimension to 128. The second convolutional layer---128 filters, kernel size~3---outputs (128, 128) after ReLU, with subsequent max-pooling to (128, 64). This hierarchical architecture operates analogously to a visual cortex for grid signals: the first layer detects fine-grained transient edges (sudden voltage steps at individual buses), and the second composes those into higher-level patterns (fault propagation signatures spanning multiple buses)~\cite{ref26}.
...
Training employs the Adam optimizer with a step decay schedule: epochs 1--10 at $\alpha = 0.001$, epochs 11--25 at $\alpha = 0.0005$, and epochs 26--50 at $\alpha = 0.0001$. This warm-start and gradual refinement strategy prevents training oscillations during early epochs while enabling fine-grained convergence in later stages. Early stopping monitors validation classification accuracy, terminating training after 5 consecutive epochs of no improvement. Training converges in approximately 15.7~minutes on an NVIDIA RTX~3090 GPU (batch size~64, 60,000 examples). Examples are shuffled each epoch to break temporal correlations between faults originating from the same simulation event.
...
The ablation results in Table~\ref{tab:latency} provide insight into each architectural component's contribution. Removing the LSTM component (CNN-only) reduces accuracy by 4.68~points (96.78\% $\rightarrow$ 92.1\%), confirming that temporal modeling is essential for distinguishing faults with similar spatial signatures but different dynamic trajectories---high-impedance faults versus load transients, in particular. Removing the CNN component (LSTM-only) reduces accuracy by 2.58~points (96.78\% $\rightarrow$ 94.2\%), confirming that spatial feature extraction contributes meaningfully to inter-bus correlation analysis. Replacing both with a generic MLP further reduces accuracy to 93.4\%, validating the importance of task-specific inductive biases.

One finding surprised us. Models trained with classification loss alone achieved 96.1\%---0.68~points below the dual-head variant. The auxiliary action-prediction task appears to act as an effective regularizer, pushing the shared representation toward features useful for both classification and control. We did not predict this effect during the architecture design phase; it emerged from the experiments, and we are still working through the theoretical implications~\cite{ref28}.
...
The architecture succeeds because it mirrors the physics of the problem---and that claim is supported by the evidence, not just asserted. Faults in electrical networks exhibit a characteristic spatio-temporal signature: a disturbance originates at a specific location, propagates through the transmission network along paths determined by line impedances, and evolves temporally according to the electromechanical dynamics of generators and loads. The CNN captures spatial propagation patterns through translation-invariant filters that learn characteristic inter-bus voltage correlation signatures regardless of which specific bus is faulted. The LSTM captures temporal evolution through its gated memory architecture, compressing the fault's dynamic trajectory over the analysis window into a compact latent representation~\cite{ref11,ref26}.
...
Fourth, and most critically for practical deployment: the edge deployment numbers presented in this paper are projections, not measurements. We have validated the model architecture and accuracy on GPU hardware. The Pi~5 latency figures are derived from quantization analysis and operator fusion studies on comparable architectures. Hardware-in-the-loop validation on an actual Raspberry Pi~5 remains essential future work before any field deployment can be contemplated. We are explicit about this boundary between measured and projected results because the distinction matters for safety-critical infrastructure.

\subsection{Hardware Implementation Considerations}

The projected edge deployment results on the Raspberry Pi~5 suggest that substation-level installation without GPU-accelerated hardware is feasible. The expected 2.5~W power consumption during inference would enable battery-backed operation exceeding 48~hours during station supply failures. The INT8-quantized model footprint of 2.1~MB is readily accommodated on available storage, and the peak inference memory of approximately 178~MB (batch size~1) operates comfortably within the Pi~5's 4~GB RAM constraint. For higher-performance deployment scenarios, migration to an NVIDIA Jetson Orin Nano (40~TOPS, 15~W) is projected to reduce inference latency well below 1~ms, opening the door to sub-5~ms end-to-end detection inclusive of PMU sampling delay. The model's compatibility with ONNX and TensorFlow Lite export formats further broadens deployment flexibility across embedded platforms~\cite{ref29,ref30}.

\subsection{Integration with Protective Relays}

Let us be clear about what System-1 is and is not. It is not a replacement for existing protective relays. It is an advisory layer that runs in parallel---a belt-and-suspenders philosophy consistent with NERC reliability standards~\cite{ref37}. The integration architecture places System-1 as a supplementary detection layer providing rapid fault classification and preliminary action recommendations to the existing SCADA/EMS infrastructure. A confidence threshold of 0.95 on the softmax output serves as the decision gate: if System-1's classification confidence exceeds this threshold and the detected fault type requires immediate action, a protection command is issued; otherwise, the system defers to the traditional relay coordination scheme. This architecture ensures that any failure of the neural network component simply results in reversion to proven relay-based protection~\cite{ref23}. Regulators need to know---and we want to be on record stating---that adding a neural network does not, and must not, subtract reliability.

\section{Conclusion and Future Work}
\label{sec:conclusion}

The bottom line is this: we have demonstrated that sub-10~ms fault detection is achievable on GPU hardware, and that the same architecture, once quantized and optimized, is projected to achieve comparable performance on low-cost edge devices. System-1 delivers 96.78\% $\pm$ 0.4\% average detection accuracy (95\% CI: 95.9\%--97.6\%) across seven fault categories on the IEEE~39-bus test system, with $3.2 \pm 0.6$~ms measured inference latency on an NVIDIA RTX~3090 GPU. The architecture outperforms traditional distance relays by 8.2$\times$ in speed and 11.6~points in accuracy. It maintains above 92\% accuracy under adversarial FDI attacks, and ablation studies confirm that both the CNN and LSTM pathways are individually necessary. Cross-system generalization to the 118-bus benchmark reaches 89.45\% under zero-shot transfer, with transfer learning projected to recover above 94\%. The projected edge deployment on a Raspberry Pi~5 yields $8.6 \pm 0.4$~ms latency at 96.27\% accuracy---numbers that await hardware validation.
...
Three open questions keep us up at night and deserve the field's attention. First, can System-1 detect fault types it has never seen? Arc flash events and transformer saturation signatures were not in our training set, and we do not know how the model would respond. Meta-learning approaches that enable rapid adaptation to novel fault categories with minimal additional data could close this gap. Second, what is the theoretical lower bound on detection latency? Our measured 3.2~ms GPU time is fast, but is it fast enough? An information-theoretic analysis of the minimum measurement window required for reliable fault discrimination would tell us whether we are pushing against a fundamental limit or merely the limits of our current implementation. Third---and this one matters critically for regulatory acceptance---we need formal probabilistic bounds on false-trip rates. A verification framework combining conformance checking with statistical model checking could provide the guarantees that safety-critical deployment demands. We are working toward being the ones who establish those guarantees.
